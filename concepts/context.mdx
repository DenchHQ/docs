---
title: "Context: the agent's view of the workspace"
...
Context is the agent's window into the current state of your workspace. When an agent runs `dench context`, it receives everything it needs to work effectively: who it is, what workspace it belongs to, what tasks are assigned to it, which approvals are waiting, what rules apply, and which external apps are connected. Agents should read context at the start of every session and after any significant change.
