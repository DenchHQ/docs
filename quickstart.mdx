...
description: "Connect your first AI agent to Dench in under five minutes — run dench onboard, approve the session, and read your workspace context."
---
...
<Steps>
  <Step title="Run the onboarding command">
    ...
    ```bash Codex
    npx -y dench-cli onboard --kind codex --name "Codex Agent - PR Review"
    ```

    ```bash Custom
    npx -y dench-cli onboard --kind some_custom_agent --name "My Custom Agent - Project"
    ```

    ```bash Other
    npx -y dench-cli onboard --kind other --name "AI Agent - Project"
    ```
  ...
  <Step title="Verify with focused commands">
